...
import wxdgaming.spring.boot.starter.core.lang.Tuple2;
import wxdgaming.spring.boot.starter.core.loader.ClassDirLoader;
import wxdgaming.spring.boot.starter.core.loader.RemoteClassLoader;
...
public class ReflectContext {
    ...
    public <U> Stream<Class<U>> classWithSuper(Class<U> cls, Predicate<Class<U>> predicate) {
        ...
        Stream<Class<U>> tmp = stream().filter(c -> c.withSuper(cls)).map(c -> (Class<U>) c.instance);
    ...
    public Stream<Class<?>> classWithAnnotated(Class<? extends Annotation> annotation, Predicate<Class<?>> predicate) {
        Stream<Class<?>> tmp = stream().filter(content -> content.withAnnotated(annotation))
    ...
    public Stream<Tuple2<Class<?>, Method>> withMethodAnnotated(Class<? extends Annotation> annotation, Predicate<Tuple2<Class<?>, Method>> predicate) {
        Stream<Tuple2<Class<?>, Method>> methodStream = stream().flatMap(info -> info.methodsWithAnnotated(annotation).map(m -> new Tuple2<>(info.instance, m)));
        if (predicate != null) {
    ...
    public static class Content<T> {
        ...
        Content(Class<T> instance) {
            this.instance = instance;
            this.fields = Collections.unmodifiableCollection(FieldUtil.getFields(false, instance).values());
            this.methods = Collections.unmodifiableCollection(MethodUtil.readAllMethod(false, instance).values());
        }

        /** 是否添加了注解 */
        public boolean withAnnotated(Class<? extends Annotation> annotation) {
            return AnnUtil.ann(instance, annotation) != null;
        }

        /** 是否添加了注解 */
        public boolean withSuper(Class<?> cls) {
            return cls.isAssignableFrom(this.instance);
        }
