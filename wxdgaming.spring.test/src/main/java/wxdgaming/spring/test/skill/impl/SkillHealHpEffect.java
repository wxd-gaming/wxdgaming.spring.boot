...
public class SkillHealHpEffect extends AbstractSkillEffect {
    ...
    public SkillHealHpEffect(SkillCfg skillCfg, TargetGroup targetGroup, int targetCount) {
        super(skillCfg, "回血", targetGroup, targetCount);
    }
    ...
    @Override protected void onExecute(MapObject self, MapObject target) {
        ...
        log.debug(
                "{}使用了技能{}({})，恢复了{} {}点生命值, 当前生命值：{}",
                self.getName(), getSkillCfg(), getName(), target.getName(), random, target.getHp()
        );
    }
