...
@Slf4j
@Getter
public class SkillHealHpEffect extends AbstractSkillEffect {


    public SkillHealHpEffect(SkillCfg skillCfg, TargetType targetType, List<SkillCost> costs) {
        super(skillCfg, "回血", targetType, costs);
    }
    ...
    @Override protected void onExecute(Character caster, Character target) {
        ...
        log.debug("{}使用了技能{}({})，恢复了{} {}点生命值, 当前生命：{}", caster.getName(), getSkillCfg(), getName(), target.getName(), random, target.getHp());
    }
